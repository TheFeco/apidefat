...
if ($_POST['METHOD'] == 'POST') {
    ...
    function generarHTMLColumna($datos)
    {
        $html = '';
        //foreach ($datos as $row) {
        // Genera el HTML para cada registro
        // Asegúrate de usar las mismas etiquetas y estilos que usaste anteriormente
        $html .= '
            <tr>
                <td>
                    <img src="' . $datos["foto"] . '" class="imagen" height="100" width="100">  
                    <h3>' . $datos["funcion"] . '</h3>
                    <h3>' . $datos["rama"] . '</h3>
                </td>
                <td>
                <div class="job">
                <span style="display: inline-flex"><span><h3>Apellido(s): </h3></span><span><p>' . $datos["apellidos"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>Nombre(s): </h3></span><span><p>' . $datos["nombre"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>Fecha de nacimiento: </h3></span><span><p>' . $datos["fh_nacimeinto"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>CURP: </h3></span><span><p>' . $datos["curp"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>Escuela: </h3></span><span><p>' . $datos["escuela"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>Turno: </h3></span><span><p>' . $datos["turno"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>Entidad: </h3></span><span><p>Sinaloa</p></span> </span>
                <span style="display: inline-flex"><span><h3>Municipio: </h3></span><span><p>' . $datos["municipio"] . '</p></span> </span>
                <span style="display: inline-flex"><span><h3>Zona Escolar: </h3></span><span><p>' . str_pad($datos["zona"], 2, "0", STR_PAD_LEFT) . '</p></span> </span>
            </div>	
                </td>
            </tr>
            <tr class="espacio">
    ...
    // Organizar los deportistas en filas y columnas
    $htmlFinal = '<table>';
    $deportistasPorFila = 0;
    $filasPorPagina = 0;
    $htmlFinal .= '<tr>'; // Iniciar una nueva fila
    foreach ($data as $index => $deportista) {
        if ($deportistasPorFila == $colums || $index == count($data)) {
            $htmlFinal .= '</tr>'; // Cerrar la fila actual
            $deportistasPorFila = 0;
            $htmlFinal .= '<tr>'; // Iniciar una nueva fila
            $filasPorPagina++;
            if ($filasPorPagina == 3) {
                $filasPorPagina = 0;
                $htmlFinal .= '</table><div style="page-break-after: always;"></div><table>'; // Agregar un salto de página después de 6 deportistas
            }
        }
        $htmlFinal .= generarHTMLColumna($deportista);
        $deportistasPorFila++;
    }
    $htmlFinal .= '</tr>'; // Cerrar la última fila
    $htmlFinal .= '</table>';
    ...
    <div id="inner2">
        ...
        <h3 class="centerText">Director de la escuela ó Supervisor</h3>	
